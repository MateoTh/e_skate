...
class _DetailsPageState extends State<DetailsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.skate.brand} ${widget.skate.name}'),
        actions: [
          OwnedButton(
            skate: widget.skate,
          ),
...
class OwnedButton extends StatefulWidget {
  final Skate skate;
  const OwnedButton({super.key, required this.skate});

  @override
  State<OwnedButton> createState() => _OwnedButtonState();
}

class _OwnedButtonState extends State<OwnedButton> {
  @override
  Widget build(BuildContext context) {
    DataRepository repository = DataRepository();
    return StreamBuilder<User?>(
      ...
      builder: (context, snapshot) {
        ...
        return IconButton(
          onPressed: (() => {
                if (widget.skate.owners.contains(snapshot.data!.uid))
                  {
                    widget.skate.owners.remove(snapshot.data!.uid),
                    repository.skates.doc(widget.skate.uid).update({
                      'owners': FieldValue.arrayRemove([snapshot.data!.uid])
                    }),
                    setState(() => {})
                  }
                else
                  {
                    widget.skate.owners.add(snapshot.data!.uid),
                    repository.skates.doc(widget.skate.uid).update({
                      'owners': FieldValue.arrayUnion([snapshot.data!.uid])
                    }),
                    setState(() => {})
                  }
              }),
          icon: Icon(
              widget.skate.owners.contains(snapshot.data!.uid)
